...
| :---------- | :--------- | :-----------------------
| `debug`     | `Boolean`  | Whether to enable the `observer`
| `observer`  | `Object`   | Track Adminterface's JavaScript instances if `debug = true`
| `meta`      | `Object`   | Configurations passed from the backend to the frontend
| `clear`     | `Function` | Clears the `observer` object

```javascript title="Inside your browser JavaScript console"
// Enables the debugger
> adminterface.debug = true

// Check the adminterface object
> adminterface
{debug: true, observer: {…}, meta: {…}, clear: ƒ}

// Check the observer object
> adminterface.observer
{inputs: {…}, flatpickr: Array(1), confirmDialog: ConfirmDialog}
