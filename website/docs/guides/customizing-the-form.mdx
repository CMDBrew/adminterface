---
sidebar_position: 2
description: ActiveAdmin Bootstrap Guide for Form, Inputs, Add-ons, and Nested Resources.
---
...
<Tabs
  ...
  Please visit [ActiveAdmin] for basic configurations and usage guides.
...
### DatePicker
`:date_picker` - a date select. Alternative to `:date` using the [Flatpickr] library replacing the jQuery date picker in [ActiveAdmin]
...
Check the [Inputs](#inputs) section for inputs work with the add-ons.

:::

### Input Group
We've added support for [Bootstrap/input-group]. Use the `append` and `prepend` options. When you pass in either of them, it will wrap the input inside the `.input-group` element.
...
The `has_many` fields do not support Arbre elements natively. See [Github issue #4043](https://github.com/activeadmin/activeadmin/issues/4043).

:::

Here are a couple of example workarounds:
